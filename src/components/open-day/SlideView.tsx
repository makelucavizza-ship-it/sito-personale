...
function IconChip({ icon: Icon, accent, size = 56 }: { icon: typeof Sparkles; accent: string; size?: number }) {
  return (
    <div
      className="w-20 h-20 md:w-32 md:h-32 rounded-3xl flex items-center justify-center flex-shrink-0"
...
function SlideContent({ slide, accent }: { slide: OpenDaySlide; accent: string }) {
    ...
    return (
      <div className="flex flex-col items-center gap-12 w-full max-w-5xl">
        <div className="flex items-center justify-center gap-4 md:gap-10 flex-wrap">
          {icons.map((Icon, i) => (
            <div key={i} className="flex items-center gap-4 md:gap-10">
              <IconChip icon={Icon} accent={accent} size={44} />
              {i < icons.length - 1 && <ArrowRight size={28} className="opacity-20 hidden md:block" />}
            </div>
          ))}
        </div>
        <p className="text-2xl md:text-4xl text-center opacity-70 px-4" style={{ fontFamily: "Sailors, Georgia, serif" }}>
          {parts.join(" → ")}
        </p>
      </div>
    );
  ...
  if (slide.icon === "numbers") {
    ...
    return (
      <div className="flex flex-col md:flex-row items-center gap-8 md:gap-20">
        <p className="text-6xl md:text-8xl lg:text-9xl font-bold text-[#5bc783]" style={{ fontFamily: "Phenomena, sans-serif" }}>
          {left}
        </p>
        <p className="text-6xl md:text-8xl lg:text-9xl font-bold text-[#ee826d]" style={{ fontFamily: "Phenomena, sans-serif" }}>
          {right}
        </p>
      </div>
    );
  ...
  if (slide.icon === "quote") {
    return (
      <p
        className="text-4xl md:text-7xl lg:text-8xl text-center leading-tight px-4 max-w-6xl"
      ...
      <div className="flex flex-col items-center gap-8 w-full">
        {isContain ? (
          ...
          <img
            src={slide.image}
            alt={slide.keyword}
            className="max-h-[68vh] w-auto rounded-2xl shadow-2xl object-contain"
          />
        ) : (
        ...
        <p className="text-3xl md:text-5xl font-bold text-center" style={{ fontFamily: "Phenomena, sans-serif" }}>
          {slide.keyword}
        </p>
      </div>
    );
  ...
  return (
    <div className="flex flex-col items-center gap-8 text-center px-4 max-w-5xl">
      {Icon && <IconChip icon={Icon} accent={accent} />}
      <p className="text-6xl md:text-8xl lg:text-9xl font-bold" style={{ fontFamily: "Phenomena, sans-serif" }}>
        {slide.keyword}
      </p>
      {slide.lines?.map((line, i) => (
        <p key={i} className="text-2xl md:text-4xl opacity-60" style={{ fontFamily: "Sailors, Georgia, serif" }}>
          {line}
        </p>
      ))}
    </div>
...
export default function SlideView({
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center gap-10 px-4">
